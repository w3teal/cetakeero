...
bool print_job::print_pdf(const gchar* name,
  ...
  if (printer != nullptr) {
    ...
    // Initialize the dialog's page setup with the page size and margins
    // requested by the application (in points) so the printer uses the same
    // page size as the app, instead of the printer driver's default.
    if (pageWidth > 0 && pageHeight > 0) {
      GtkPageSetup* setup = gtk_page_setup_new();
      GtkPaperSize* paper = gtk_paper_size_new_custom(
          "cetakeero", "cetakeero", pageWidth, pageHeight, GTK_UNIT_POINTS);
      gtk_page_setup_set_paper_size(setup, paper);
      gtk_paper_size_free(paper);
      gtk_page_setup_set_orientation(
          setup, pageWidth > pageHeight ? GTK_PAGE_ORIENTATION_LANDSCAPE
                                        : GTK_PAGE_ORIENTATION_PORTRAIT);
      gtk_page_setup_set_left_margin(setup, marginLeft, GTK_UNIT_POINTS);
      gtk_page_setup_set_top_margin(setup, marginTop, GTK_UNIT_POINTS);
      gtk_page_setup_set_right_margin(setup, marginRight, GTK_UNIT_POINTS);
      gtk_page_setup_set_bottom_margin(setup, marginBottom, GTK_UNIT_POINTS);
      gtk_print_unix_dialog_set_page_setup(dialog, setup);
      g_object_unref(setup);
    }
